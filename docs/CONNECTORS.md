...
| Notion | Internal integration token (`ntn_…` / `secret_…`) | 25 most recently edited pages shared with the integration | Via Auxilo's worker |
| Slack | **User** token (`xoxp-…`) with `search:read` | 25 recent messages sent to you | Via Auxilo's worker |

## Why two of them need a proxy

GitHub and Linear send permissive CORS headers, so the browser calls them
directly and the key never touches Auxilo's server.
