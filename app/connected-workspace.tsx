  ...
  return <section className="source-connections" aria-label="Calendar and contact connections">
    ...
    <div className="source-grid">
      <section className="module-card source-panel">
        <div className="source-panel-heading"><span className="connection-logo calendar">G</span><div><h2>Google Calendar &amp; Contacts</h2><p>{s.isNativeShell ? "Use iPhone Calendar below on this device" : s.sessions.length ? `${s.sessions.length} authorized account${s.sessions.length > 1 ? "s" : ""}` : "Not connected"}</p></div></div>
        {s.isNativeShell ? <p>Google sign-in can&apos;t run inside the app. Connect Google on the web at auxilo.app, or read your iPhone calendar and contacts directly below — no Google needed.</p> : <>
